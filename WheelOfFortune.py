...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        response = input(self.name + " has $"+ str(self.prizeMoney)+ "\n" + "Category: " + category + "\n" + "Phrases:" + "\n" + obscuredPhrase + "\n" + "Guessed:" + guessString + "\n" + "Guess a letter, phrase, or type 'exit' or 'pass':")
        return(response)
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        possible = []
        for char in LETTERS:
            if self.prizeMoney >= 250 and char not in guessed:
                possible.append(char)
            elif self.prizeMoney < 250 and (char not in guessed and char not in VOWELS):
                possible.append(char)
        return possible
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if self.prizeMoney < 250:
            ...
            if v_list == p_vowels:
                return 'pass'
        if coinflip == True:
            for s_guess in self.SORTED_FREQUENCIES:
                if s_guess in possible:
                    return s_guess
        elif coinflip == False:
